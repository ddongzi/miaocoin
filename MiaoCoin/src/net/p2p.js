...
class P2P {
  ...
  updatePeers(peers) {
    ...
    // 更新对等节点列表，去重
    this.peers = Array.from(new Set(this.peers.concat(peers)));
  ...
  broadcast(msg) {
    console.log(
      `broadcast ${JSON.stringify(msg)}, sockets : ${this.peerSockets.length}`
    );
    // 向各peer服务地址发出请求
    this.peerSockets.forEach((socket) => socket.send(JSON.stringify(msg)));
  }
}
